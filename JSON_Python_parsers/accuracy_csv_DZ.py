...
outputFName = inputCSV + "_ACCRBEHAV.csv"
path = os.getcwd()
...
class Trial:
    ...
    def SetAgentBehaviour(self, agentBehaviour):
        self.agentBehaviour = agentBehaviour
...
class Participant:
    ...
    def GetAccuracy(self, task, workload, displayMode, agentBehaviour):
        isCorrectTotal = 0
        totalCount = 0
        #print("Average for: ", task, " and ", workload, " and ", displayMode)        
        for t in self.trials:            
            if t.task == task and t.workload == workload and t.displayMode == displayMode and t.agentBehaviour == agentBehaviour:
                totalCount += 1                
    ...
    def GetReactionTime(self, task, workload, displayMode, agentBehaviour):
        count = 0
        timeTotal = 0
...
if __name__ == '__main__':
    ...
    for line in lines:
        ...
        newTrial.SetAgentBehaviour(components[9])
    ...
    newLine = "\n"
    rowHeaders = "participantID,participantIndex,task,maxDroneCount,displayMode,agentBehaviour,accuracy,reactionTime,a2rt" + newLine
    ...
    for p in participantList:
        ...
        #for w in ("4","8","12"):
            ...
            #for d in ("Text","Graphical"):                
                displayMode = d + ","
                for b in ("Correct Diversion", "Incorrect Diversion", "Correct Incursion", "Incorrect Incursion"):
                #print(d, w, participantInfo)
                    agentBehaviour = b + ","
                    a = p.GetAccuracy("Danger Zone", w, d, b)
                    rt = p.GetReactionTime("Danger Zone", w, d, b)
                    accuracy = str(a) + ","
                    reactionTime = str(rt) + ","
                    a2rt = str(a/rt)
                    csvBuffer += participantInfo + workload + displayMode + agentBehaviour + accuracy + reactionTime + a2rt + newLine
